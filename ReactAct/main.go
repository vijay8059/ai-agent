...
import (
	...
	"github.com/vijay8059/ai-agent/ReactAct/agent"
...
func main() {
	...
	// Stream every step to the terminal so you can watch the agent think.
	a.OnStep = func(s agent.Step) {
		switch s.Type {
		case agent.StepThink:
			fmt.Printf("\n\033[33m[THINK]\033[0m %s\n", s.Content)
		case agent.StepAct:
			fmt.Printf("\n\033[36m[ACT  ]\033[0m %s\n", s.Content)
	...
	fmt.Println("AI Agent ready. Type your task and press Enter.")
